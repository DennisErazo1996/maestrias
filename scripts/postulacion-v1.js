...
function formValidation(){
    ...
    let dedicacion = $('input[name=dedicacion]:checked').val();
    ...
    let letlecturaEspaniol = $('#letlecturaEspaniol').val();
    let lecturaIngles = $('#lecturaIngles').val();
    let lecturaOtros = $('#lecturaOtros').val();
    let escrituraEspaniol = $('#escrituraEspaniol').val();
    let escrituraIngles = $('#escrituraIngles').val();
    let escrituraOtros = $('#escrituraOtros').val();
    let conversacionEspaniol = $('#conversacionEspaniol').val();
    let conversacionIngles = $('#conversacionIngles').val();
    let conversacionOtros  = $('#conversacionOtros').val();
    ...
    if(!checkName(apellido_paterno)){
    ...
    } else if(!checkString(salario)){
        showValidationMsj('#salario', validacion, 'Escriba su salario');
    } else if(dedicacion === undefined){
        showValidationMsj('input[name=dedicacion]', validacion, 'Seleccione un porcentaje de tiempo');
        ...
        showValidationMsj('#p1', validacion, 'Escriba por qué ha decidido estudiar la Maestría');
    } /*else if(!checkSelect(p2)){
        showValidationMsj('#p2', validacion, 'Seleccione qué orientación de la Maestría le interesa cursar');
    }*/ else if(!checkSelect(p3)){
        showValidationMsj('#p3', validacion, 'Selecciones qué tipo de Maestría le interesa cursar');
        ...
        data.append('dedicacion', dedicacion);
        ...
        data.append('p4', p4);
        data.append('p5', p5);
        data.append('maestria', maestria);
        data.append('pdf', pdfFile);

        $.ajax({
			url:'process/postulacion',
			data:data,
			type:"POST",
			processData: false,
    		contentType: false,
